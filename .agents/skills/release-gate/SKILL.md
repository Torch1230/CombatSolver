...
客户端 0.35.3 起随心跳检查监控系统维护的版本。发布顺序固定为：**创意工坊上传成功 → GitHub Release 发布完成且最小 ZIP 上传成功 → 最后更新监控系统 `latestVersion`**。两个渠道必须是同一版本，并各自已有成功证据，才可推进更新提醒；任一渠道未完成或失败，后台保持原版本，避免工坊玩家收到提醒却下载不到新版。仅 GitHub 发布成功、准备 ZIP、创建标签或推源码均不满足更新提醒条件。

单独要求上传某一渠道时，只执行已授权的渠道操作；另一渠道没有同版本成功证据就保持后台原值，不擅自扩大发布范围。复用本次同版本已有成功记录，不重复上传或写入版本。
...
任一渠道未完成时保持原已发布版本。两个渠道均成功但监控写入失败时，明确报告“渠道已发布，监控版本未更新”及失败原因，修正明确原因后仅重试该步骤，不重复构建/上传。撤回提醒可用 `off`；不能从创意工坊查询接口、客户端上报数量或本地最大版本号猜测线上最新版。
