...
- `NoveltyPortfolio` 是默认开启、可由玩家关闭的主搜索组合，不是新评分或状态等价关系。探索和Beam共享请求节点/时间，只共享完成结果，保留独立转置与frontier；预算上限、章节首领较小探索份额和短预算旁路由 `NoveltyPortfolioBudget` 决定。不能恢复实验中的无限OPEN或为每个成员重开总预算。默认参数和反例见 `docs/strategy/bounded-novelty-search-20260916.md`。
