...
- Windows 搜索内后台回收由诱发收集直接结束当前 NoGC，避免 EndNoGCRegion 与收集请求之间的分配触发前台收集；进入区域前设置 SustainedLowLatency，单独保留并恢复原延迟模式。区域预留沿用上游用户预算与安全准入约束，不增加证据不足的Windows 4GB硬上限；碎片不抵扣新预留的物理余量。区域重建也可暂停托管线程且不计入 GetTotalPauseDuration，暂停证据须同时核对 CLR SuspendEE/RestartEE 与重建耗时。完成哨兵、排空、取消及手动请求边界仍适用；区域请求预算不是进程内存上限。
