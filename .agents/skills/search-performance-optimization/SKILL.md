...
紧凑候选接入阶段：只有主线程捕获的 `CompactCombatRoot` 可以通过 policy 注入；Runtime 启用需要单独完整闭包验收。每固定 lane 独占执行与读取状态，冻结候选只持有不可变值和根，不得持有读取器／祖先链。快照按需拥有的兼容图必须与权威值分开并一同释放；种子校验精确父候选。缺选择时保留原挂起语义，回退前不消费逻辑计数。比较完整路线、全部最终评分字段、展开／转移／选牌／剪枝计数；物理前缀恢复和兼容图数量另计。当前完整机甲搜索逻辑已一致但速度回退，不能据此宣称优化完成，仍须消除热点及验证原始亡灵完整输入。
