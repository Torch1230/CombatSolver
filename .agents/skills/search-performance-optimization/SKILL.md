...
- 值 RNG 的整池选取只使用调用者独占 scratch，选中前缀在复用前必须消费或冻结。原生 TakeRandom 即使请求 0／负数也洗乱整个池；请求超过池大小则返回全池。空／单元素池不推进 RNG。完整池原生对照与零分配原语测试不等于生成命令、回调、候选冻结或正常 NoGC 性能验收。

- 原生随机生成池只在主线程根捕获解锁与资格，完整角色池和攻击投影保持原候选顺序，Fork 共享只读数据。不要把 CallOfTheVoid 的逐层 `TakeRandom(1)` 换为一次随机索引或一批互异抽取：原生每层洗乱整池，跨层允许重复。选取 RNG 与生成卡实例属于分支；非规范／自定义牌池不得命中原生缓存。新增池准入必须验证全部候选，不按已迁移卡牌缩池。
...
一次性附魔可沿用实例的可撤销槽，但必须证明定义编号、X、关键字和失效位互不覆盖，生成身份也遵守同一编码。抽牌暂停保存附魔内部执行位置，不能重启或跳过剩余工作；每个读视图私有导入状态并清理卡牌缓存。开局能力前缀的语义测试与候选保留质量分开计量，不能通过扩大候选配额让 fixture 通过；固定搜索仍对照原政策、完整结果和串并行。
