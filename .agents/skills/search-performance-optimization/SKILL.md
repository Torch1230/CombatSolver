...
- 循环终局探测仅在动作层的 worker 全部排空后按原序执行。只对已满足策略目标、零累计掉血/卖血、每步唯一可执行身份和目标的延续作真实回放；保留普通出牌和 EndTurn 出口，选牌/资源下降/漂移立刻停止额外探测，只有真实胜利交付。4096 动作是每次 Solve 的总探测上限，不是扩大全局节点额度；关闭达标早停不得支付该成本。DOP 对账排除本来分 lane 的缓存容量与调度计数，动作、评分、展开、转移和政策剪枝必须相同。有效格挡按当前投影吸收量及原规则可保留量计，固定保留上限复用清空执行的纯计算，不能退回裸 PlayerBlock 作正向进展。
