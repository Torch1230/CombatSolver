...
- 上下文排序实验只通过显式不可变 profile 注入，默认关闭；加载和程序集校验在搜索前，内环不读文件或 live 设置。修正只影响中途排序，不能进入终局政策、状态等价或精确支配。训练只用有完整后续见证且同根同预算的候选；没有后续的被剪节点保持未知，旧 Score 尾键差异不作为政策收益标签。范围回退、正则和小幅修正都不能证明质量不降；须在训练外根和真实协调器上验收，出现胜负翻转退化不得上线。采集诊断不能作为性能样本。工具与首轮反例见 `tools/ContextualOrdering/README.md`。
