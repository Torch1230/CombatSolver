...
准入与模型适配现统一位于 `src/Prediction/Compact/`；纯值执行仍属于 Engine，估值／政策仍属于 Search，原生差分与计量仍属于 Testing。路径迁移不表示后端已启用；在搜索接线通过完整合同前，双端门禁继续禁止 Search／Runtime 调用这些适配器。

紧凑候选接入阶段：只有主线程捕获的 `CompactCombatRoot` 可以通过 policy 注入；Runtime 启用需要单独完整闭包验收。每固定 lane 独占执行与读取状态，冻结候选只持有不可变值和根，不得持有读取器／祖先链。快照按需拥有的兼容图必须与权威值分开并一同释放；种子校验精确父候选。缺选择时冻结执行位置并按原挂起边界提交逻辑计数；请求与选项预览须由快照独占。比较完整路线、全部最终评分字段、展开／转移／选牌／剪枝计数；物理前缀恢复和兼容图数量另计。完整机甲搜索逻辑一致及 headless 加速只证明该已准入域，仍须验证原始亡灵完整输入与最终 Runtime 接入。
