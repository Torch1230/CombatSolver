...
- 循环折叠保留完整平坦动作，不复用 ReplayCount 表示周期次数。snapshot 的动作结构身份及全部显示值/语言身份共同决定可折叠性；路线行保存区间、周期与偏移，真实动作数和部署索引保持原合同。末击/遗物效果/选择不同必须分开，独立循环标签随语言通知刷新。16 行之外须提示未显示回合数。验证区间中间/末次/后缀高亮、失败构造重试和折叠行复用。
