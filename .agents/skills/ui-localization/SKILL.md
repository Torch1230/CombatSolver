...
- 循环折叠保留完整平坦动作，不复用 ReplayCount 表示周期次数。snapshot 的动作结构身份及全部显示值/语言身份共同决定可折叠性；路线行保存区间、周期与偏移，真实动作数和部署索引保持原合同。末击/遗物效果/选择不同必须分开，循环次数与整周期动作包在同一轻量外框，次数放在动作右侧；宽面板按内容收紧，窄面板在组内换行，不新增动作间箭头。路线外层布局区分自然宽度和最小宽度，避免长循环阻止面板收窄；悬停说明随语言通知刷新。16 行之外须提示未显示回合数。验证区间中间/末次/后缀高亮、失败构造重试和折叠行复用。
