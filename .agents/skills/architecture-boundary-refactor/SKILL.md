...
紧凑冻结状态由候选独占页表并共享不可变值页；工作区的页缓存只是派生数据，写入和 rollback 均须失效。恢复必须在写前拒绝外根与活动 checkpoint，页表不能指回 worker 或祖先候选。页共享与零分配恢复不等于生产 Beam 存活量或 GC 暂停改善；长历史仍需核对页表、恢复扫描和所有实际语义消费者。
