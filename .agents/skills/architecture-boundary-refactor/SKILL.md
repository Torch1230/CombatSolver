...
- headless 平台脚本中，实例目录归属、完整游戏/Mod 内容快照和主机资源预约属于 `tools/headless-runtime.ps1/.sh`；启动器保留请求协议、精确 PID/出生身份终止、结果与静稳 ACK。不得把这类测试协调放入游戏 Search/Runtime，或只移除全局进程检查而继续共享 DLL/协议文件。并行只作正确性/吞吐验证，性能结论保留独占口径。
