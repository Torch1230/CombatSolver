...
`GenerationPoolChecks.RunBeforeHandDraw` 直接驱动真实回合入口：施加并叠加的 CallOfTheVoid 计数、施加者与获得顺序、生成事件排在起手抽牌之前、五字段 RNG 消耗 `count×(池−1)`、撤销与八工作区恢复、零层不推进随机流、满手溢出按选择顺序进入弃牌堆，以及缺池／缺玩家槽／越界池索引的构造拒绝。它是纯值合同，不代表生产准入已扩大；原生差分与完整池拒绝见[虚空之唤报告](../../docs/performance/simulation-void-generation-20260911.md)。
...
`PanacheChecks` 覆盖独立实例与获得顺序、嵌套完成、多目标离场、回合重置、300 实例增长、撤销和八工作区恢复。合成指令只证明布局合同，真实原生卡牌、创建后挂起、终局、完整读取与固定搜索见[神气制胜报告](../../docs/performance/simulation-panache-20260911.md)。

毁灭回合合同另核对待失败后敌方能力快照、清格挡与新分派门禁；敌方延迟格挡仅属纯值内核向量，不表示该主人组合已通过根准入。

属性能力重获向量区分人工制品的请求量类型与持续标记的原生类型：玩家负力量／负敏捷不创建持续跳过标记，重新获得虚弱仍创建该标记。原生组合见[双方力量削减报告](../../docs/performance/simulation-shared-fate-20260911.md)。
