...
`BufferContracts` 以三个独立列表作 oracle，覆盖 800 步交错分配／写入／嵌套撤销／冻结恢复，八工作区逆序读取，64／2048／65536 附近的索引层级边界，以及事件中完整有符号 32 位实例、目标、金额和标记的往返。见[索引缓冲区报告](../../docs/performance/simulation-indexed-buffer-20260911.md)。
