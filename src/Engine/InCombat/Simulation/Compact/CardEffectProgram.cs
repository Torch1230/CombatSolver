            ...
            switch (instruction.Kind)
            {
                ...
                case CardInstructionKind.ApplyBasicPower:
                    if (instruction.Target is not (CardInstructionTarget.Owner or CardInstructionTarget.ChosenEnemy or CardInstructionTarget.AllEnemies)
                        || instruction.Power is not (BasicPowerKind.Strength or BasicPowerKind.Dexterity or BasicPowerKind.Weak or BasicPowerKind.Vulnerable or BasicPowerKind.Poison or BasicPowerKind.ToolsOfTheTrade or BasicPowerKind.Neurosurge or BasicPowerKind.BorrowedTime or BasicPowerKind.Veilpiercer or BasicPowerKind.SpiritOfAsh or BasicPowerKind.DanseMacabre or BasicPowerKind.Lethality or BasicPowerKind.Pagestorm or BasicPowerKind.CallOfTheVoid or BasicPowerKind.Doom)
                        ...
                        // Doom and Vulnerable have no admitted X-scaled command, so the
                        // whole family stays at one captured request per roster pass.
                        || instruction.Power is BasicPowerKind.Vulnerable or BasicPowerKind.Doom && (instruction.Target == CardInstructionTarget.Owner
                            || instruction.Amount < 0 || instruction.EnergyXMultiplier != 0)
                        || instruction.Power is BasicPowerKind.ToolsOfTheTrade or BasicPowerKind.Neurosurge or BasicPowerKind.BorrowedTime or BasicPowerKind.Veilpiercer or BasicPowerKind.SpiritOfAsh or BasicPowerKind.DanseMacabre or BasicPowerKind.Lethality or BasicPowerKind.Pagestorm or BasicPowerKind.CallOfTheVoid && (instruction.Target != CardInstructionTarget.Owner
                            || instruction.Amount < 0 || instruction.EnergyXMultiplier != 0))
