...
internal sealed partial class UnattendedTestRunner
{
    ...
    private static async Task AssertNoGcBudgetTransitionAsync()
    {
        ...
        try
        {
            ...
            // Windows drains the old region when its search ends; other platforms
            // retain it and rebuild it here for the changed request.
            int expectedBudgetRebuilds = OperatingSystem.IsWindows() ? 0 : 1;
            if (SearchGcPolicy.BudgetChangeRebuildCountForTesting != expectedBudgetRebuilds)
            {
                throw new InvalidOperationException(
                    $"No-GC 改预算后重建次数为 " +
                    $"{SearchGcPolicy.BudgetChangeRebuildCountForTesting}，预期为 {expectedBudgetRebuilds}。");
            }
        }
        finally
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (SearchGcPolicy.BackgroundReclaimStartedCountForTesting != 1
            || SearchGcPolicy.BackgroundGen2CompletedCountForTesting != 1
            || SearchGcPolicy.BackgroundReclaimJoinCountForTesting != (OperatingSystem.IsWindows() ? 2 : 1)
