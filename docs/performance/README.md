...
- [PR #129 上游整合与极高复测](pr129-final-validation-20260923.md)：最新上游基线、默认预算、8/16 并行与 Windows 结果。
- [游戏 DLL 辅助方法审查](native-helper-audit-20260923.md)：现有采样的归因限制和可安全修改的边界。
...
- [极高8/16路的有界决策筛查](bounded-quality-screen-20260922.md)：额度回收、连续威胁与资源后能力前缀共12个对照均未改善最终质量；原型撤回，20%目标未达成。

- [开局前缀历史与能力路线并行筛查](opening-prefix-parallel-screen-20260922.md)：修复前缀丢失导致的严格回放错误，六根极高8/16路路线等价；跨成员并行作为独立受限原型测量。

- [极高8/16路提速筛查与稳定请求判重](parallel-speed-screen-20260922.md)：撤回等待方式原型，修复可变record哈希下的一次性义务判重；20%速度/显著质量目标未达成。

- [完整回合层之间回收](turn-layer-reclaim-20260922.md)：相对 `d373bdee`，24根默认NoGC的计算期RSS峰值再降53.71%、托管占用再降60.26%，请求耗时增加3.08%；34对路线等价、64项合同，保留软预算差异和普通GC反向样本。
- [计算过程中的搜索间回收](active-search-gc-20260922.md)：17根默认NoGC下RSS峰值逐场平均−36.44%、托管峰值−41.34%，请求耗时+1.56%；28对路线等价、61项合同及3项原生检查，明确小区域反例和GC暂停成本。
- [搜索结束后释放进度引用](progress-retirement-20260922.md)：17根退休后存活托管字节均值−20.98%，五角色与DOP4、合同/路线等价和原生准备流程；明确峰值/RSS及控制器fixture限制。

- [通用优化的20%门槛筛查](general-optimization-screen-20260922.md)：两根采样与跨成员StandPat重复探针，收益依据不足、探针撤回；没有达标或广泛验收结论。

- [怪物状态指纹小集合](monster-state-singleton-20260922.md)：空/单项省去排序，77项合同、原生增量对账、三场ABBA与多项局部代价。
