...
当前 `perf/general-allocation-20260914` 已整合上游0.38.2，并完成通用分配优化及蟹战后续局部优化。用户随后授权选牌暂停与恢复窄原型，已完成投掷匕首、杂技和早有准备共用的[独立实验与对照](choice-continuation-prototype-20260914.md)，默认搜索尚未接入。旧研究分支保留为历史证据，不继续Compact／战斗执行流程移植。
