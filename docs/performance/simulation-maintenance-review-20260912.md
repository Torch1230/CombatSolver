...
最新的小改动优先级与直接测量见[局部字段与数据结构研究](simulation-data-layout-20260912.md)和[合并上游后的真实机甲场景内存与耗时对照](simulation-real-scene-memory-20260912.md)。本篇保留长期架构比较；当前先研究辅助计数表、容器包装和已准入真实场景的净收益，未启动状态族迁移。
...
这是一项架构取舍，而不是对所有输入都成立的加速承诺。2026-09-12 的当前上游真实机甲场景 A/B 已在同一请求中测到 Compact 相对模型后端约 2.20 倍搜索速度、49.6% 累计分配下降和 7.15% 结束工作集下降；这证明已准入闭合域存在实际净收益，但不能外推到亡灵、药水、复杂遗物、峰值 RSS 或可见 Steam。仍没有依据给完整迁移承诺几周、几个月或固定 token 数。眼下最经济的选择，是保留已有收益，限制新增后端范围，只给能独立证明价值的优化投入下一批工作。

## 1. 证据范围与现状

源码静态审计基线为任务分支 `2f5a6df`，已合并上游 0.36.4；真实场景跟进测量记录在提交 `4b350d6`。本报告引用截至 2026-09-12 的本地源码、已有性能与差分记录，以及新增的三次真实机甲 A/B；历史性能记录仍对应不同提交、输入和运行模式，不能互相直接拼接。[^1]
...
当前 Compact 证据如下（含 2026-09-12 当期 A/B）：

| 运行条件与范围 | 时间 | 内存口径 | 可以得出的结论 |
|---|---|---|---|
| 合并上游后的真实机甲场景，三次独立 headless coordinator A/B | 11.602 → 5.272 秒，约 2.20×（−53.9%） | 累计分配 5.074 → 2.559 GB（−49.6%）；进程 CPU 37.185 → 17.639 秒（−52.0%）；结束时工作集 1.803 → 1.674 GB（−7.15%，不是峰值 RSS） | 已准入闭合域有当期实测收益；不能外推到亡灵、药水、复杂遗物、Runtime/NoGC、可见 Steam 或峰值 |
...
这些行来自不同实验口径，不构成互相矛盾的结果。新增的三次 A/B 是当前上游同一输入下的真实直接 coordinator 测量；它支持已准入闭合域的净收益，但不支持全场景或峰值 RSS 结论。整体证据更支持“降低分配压力并控制适用范围”，而不是“新内核已经普遍快两倍”。[^7][^19][^21]
...
首先复用已有数据排除明确低收益方案；当前上游已形成一个已准入机甲根的三次真实 coordinator A/B 基线，但它仍不是 Runtime NoGC、峰值 RSS 或复杂亡灵基线。下一批只有在确实需要决定实现目标时，才补充复杂亡灵和 16/8/4 GB 内存生命周期对照。基线要能区分请求时间、worker CPU、累计分配、存活状态和峰值驻留量，并记录固定搜索工作量。对保留旧游戏规则的优化，应优先在复杂亡灵原输入上判断是否有价值，而不是重新缩成容易闭合的小战斗。
...
**大幅提速与总体省内存：已有一个当前上游闭合机甲场景的真实直接 A/B，但范围仍有限。** 三次样本中 Compact 约快 2.20 倍、累计分配少 49.6%，结束工作集少 7.15%；这不是峰值 RSS、Runtime NoGC 或复杂亡灵结果。没有证据支持全场景两倍、确定峰值降幅或完整迁移期限。

**交付目标应是可维护的净收益。** 全面迁移可以一直不完成，甚至不再作为目标；只要现有引擎继续正确、有限快路径持续有收益、公共层重复逐步减少，就已经满足性能工作的实际目的。

## 来源

本地源码静态审计均按 `2f5a6df` 阅读；到 `4b350d6` 的 `src/` 只新增单一性能预设断言修复，真实场景测量使用该修复后的源码树。历史记录保留各自提交、日期、样本与运行条件。外部滚动文档与 master/main 源码访问于 2026-09-12，其用途是验证设计机制，不用于预测本项目性能。
...
[^21]: CombatSolver，[合并上游后的真实机甲场景内存与耗时对照](simulation-real-scene-memory-20260912.md)及[结构化结果](simulation-real-scene-memory-20260912.json)，2026-09-12，直接 coordinator、普通 GC、三次独立进程 A/B；工作集为后端测量结束时读数，不是峰值 RSS。
