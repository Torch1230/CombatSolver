...
### 反向对照：去掉守卫会 OOM，而不是变快

同根（crab，beam 135，`--dop 8`）、预算拉到 200000 节点，在**完全不带内存守卫**的普通 .NET 进程里跑（`tools/OfflineSearchHarness` 不经过 `SearchGcPolicy`）：**524 秒后 `System.OutOfMemoryException`**，`reachedMilestone=M1`。同机另跑 8000 节点只用 13.13 秒、分配 5052 MB（631 KB/节点）。

这条对照说明两件事：

1. **搜索的保留集确实是无界的** —— 给它足够节点预算，它会一直吃到把进程撑爆。守卫不是多余的，它正是为了避免 OOM 才存在。
2. **两个失败模式同源**：带守卫 → 在 63 节点/秒的回收抖动里出不来路线；不带守卫 → OOM。差别只在守卫把「无界」换成了「反复拆建 NoGC 区域」，代价是 33 次检查点与 83.3 秒 GC 暂停。

也就是说，**准入判定修的是守卫自身的成本，不是保留集的无界**。后者必须由「内存是输入」来解，见文末。
