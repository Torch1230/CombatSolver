...
2. **额外的确定性击杀回放。** 新 `CombatBeamSolver.CycleReplay.cs` 在整个当前动作层完成、worker 全部排空后按原序探测。只在允许达标早停、成长/遗物/偷窃目标已满足、种子没有累计掉血或卖血、周期有真实敌方耐久下降且能量/星能不下降时启动。每一步重新检查当前牌实例、费用、目标和附加重放次数，使用原 `ReplayAction` 真正执行，保持完整父链和平坦 PlanAction。不同于直接复制结算数值，没有任何伤害、历史、RNG 外推。
   - 每一步必须只有一种可执行卡牌身份和一种目标；同状态重复牌复用普通展开的等价规则。出现不同可执行牌（包括终结牌）、多个目标、选牌、非法动作、形状漂移、跨回合、自伤或不确定结算，立即结束额外探测。普通路径和结束回合候选一直保留，只发布实际完成的胜利。
   - 每个回合/形状区域至多一次，单次 Solve 合计至多 4096 个额外真实动作；估算轮数只确定回放上限，不宣称杀得死；取消、时间、内存信号每步检查。关闭达标早停时完全不做额外回放。仍有分支的循环、卖血循环、证据不足或超过此上限的长循环继续使用原搜索，不承诺任意长循环都加速。
...
4. **状态与遥测。** `BANSHEES_CRY` 加入已有历史读者门控；没有对所有战斗无条件扩键。循环停止细分为无收益、重复预算、family 深度、出口预算，加回合层截断及回放尝试/动作/胜利计数，随 SolverResult 和无人指标传播。累计掉血原本就在转置支配标签，最终候选也不会在政策比较前按状态键压成一个；本批没有把累计掉血/回血直接加入战斗指纹，避免把策略历史当新游戏状态。此处不声称已证明所有中间保路的历史等价性。
...
DOP 对照的并行调度计数以及 worker 局部 ThreatProjectionCache 条目数按设计不同，不写成“全部非时序数字相同”。当前测量没有发现大幅收益伴随明显质量或性能退化的方案；上表保留已观测到的小额分配增加。
