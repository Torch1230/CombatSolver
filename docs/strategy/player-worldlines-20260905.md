...
|---:|---:|---:|---:|---|---|---|---|---|---|
| 1 | 104 HP | 110 → 6 | +5（报告参考） | 是（报告参考） | 存活达标（20260906）：9224f63+恢复批次，start严格恢复；历史验收政策VeryHigh/DOP4、四药Force、减战损优先，预测/实际1 HP、4药、T6、余122 HP，计划外重算0；报告人工参考差+5，缺人工完整录制；证据`.local/checkpoint-batch/worldline-resume/rank1-deploy/`；本构建该根首次搜索，缓存未单列计数；历史3 HP保留为旧证据 | AEONGLASS_BOSS | `CombatSolver-AEONGLASS_BOSS-20260903-153416-774.zip` | `combatsolver-reports-20260905-104714.zip` | 110->6 |
| 2 | 68 HP | 123 → 55 | 死亡，数值不可比较 | 否 | 路线对比中，待用户指定优化路线：实际部署T11死亡、计划外重算0，证据`rank2-visible-core-analysis/`；人工T2用STABLE_SERUM保留攻击牌，求解器T3才用药，人工T4爆发200；质变机制待结合最终保留分支分析，人工55为T8累计实际加剩余预测0。后续使用headless。此前2轮实际策略修改均未存活（20260906）；High/180秒/DOP4、通关优先、仅稳定血清Force。基线死亡/敌194；R1补Rage潜在攻击次数依赖，死亡/敌125（`83d99c169f144e8db7ba2dfe7ba0659b`）；R2优先保留预计HP更高的生命投资路线，死亡/敌247（`2b04ef3094ca4f87a1e8303358398737`）；两轮各自基于a5266b4且均撤回，缓存false；证据`.local/checkpoint-batch/worldline-resume/rank2-r1-rage-search/`及`rank2-r2-survival-search/`，对应patch同目录根；此前实际部署死亡，未把敌HP改善记为战损改善 | AEONGLASS_BOSS | `CombatSolver-AEONGLASS_BOSS-20260903-152708-532.zip` | `combatsolver-reports-20260905-104714.zip` | 沙小第知道他自己打得过120血带药战士吗？ |
| 3 | 58 HP | 61 → 3 | -3（报告参考） | 否（报告参考） | 软目标达标（20260906）：184a9cd、start严格恢复，High/180秒/DOP4、禁药、减战损优先，预测/实际6 HP、0药、T6、余75 HP，目标32 HP，计划外重算0；证据`.local/checkpoint-batch/worldline-resume/rank3-high-deploy/`，`dbfded1e15bd465ab898a6d58f577040`；报告人工参考差-3，缺完整人工录制；首次High搜索来自本构建，部署缓存命中计数未采集；旧VeryHigh超时和Short未完战结果保留 | TEST_SUBJECT_BOSS | `CombatSolver-TEST_SUBJECT_BOSS-20260902-133704-483.zip` | `combatsolver-reports-20260905-104714.zip` | 打实验体不开壁垒吃重击的来 |
...
| 5 | 49 HP | 73 → 24 | 不可比较（恢复失败） | 未验收 | 跳过：旧状态戳缺少本回合卡牌历史（20260906）；184a9cd、新入口start预检材料有效，RestoreOnly严格恢复失败，尚未搜索/部署；原包Disabled，缺项补齐方案已明确保存但未进入执行；证据`.local/checkpoint-batch/worldline-resume/rank5-restore/`，runId `dc958b1b6ca84b0795fa180752f74b73` | TEST_SUBJECT_BOSS | `CombatSolver-TEST_SUBJECT_BOSS-20260902-013258-533.zip` | `combatsolver-reports-20260905-104714.zip` | 更好的世界线 |
| 6 | 45 HP | 45 → 0 | -19（报告参考） | 否（报告参考） | 软目标达标（20260906）：184a9cd，新入口recent/start完整恢复，原包Custom/DOP8/5秒短搜20秒总预算，显式补Smart、减战损优先及阈值0；预测/实际19 HP、1药、T10、余30 HP，目标22 HP，计划外重算0；证据`.local/checkpoint-batch/worldline-resume/rank6-deploy/`，`e6f07178a5a64698a969a1655009191f`；首次搜索来自本构建，随后部署缓存命中计数未采集；manual_plus_solver及缺失原政策限制人工整场比较 | MECHA_KNIGHT_ELITE | `CombatSolver-MECHA_KNIGHT_ELITE-20260902-131216-716.zip` | `combatsolver-reports-20260905-104714.zip` | 战损45变0 |
| 7 | 45 HP | 50 → 5 | -65（报告参考） | 否（报告参考） | 按用户新指令暂停策略优化，当前先分析包2；R1手中Footwork防御收益试验仍70 HP/T8，未改善战损，已撤回，R2未进行；证据`rank7-r1-ready-defense-search/`，runId `6c82c594531c4722831f31a4d3f00053`；未达27 HP（20260906）；deddc84+开战选牌测试修正，High/180秒/DOP4、Smart、原Boss政策，start严格恢复；开战预测/实际掉血70 HP、回血21、余21，瓶中精灵1、T10、计划外重算0；首次搜索缓存false、部署缓存true；证据`.local/checkpoint-batch/worldline-resume/rank7-high-opening-search/`及`rank7-high-opening-deploy/`，部署`f73f8bcfc2644c6aae9b493833b3293b`；此前17 HP是选牌后额外手动重算，已撤销其开战基线资格；旧23 HP仅历史预测 | QUEEN_BOSS | `CombatSolver-QUEEN_BOSS-20260903-173813-639.zip` | `combatsolver-reports-20260905-104652.zip` | 不尊重灵动？ |
| 8 | 44 HP | 64 → 20 | 不可比较 | 不可比较 | 跳过：人工参考未完战（20260906核对原始材料）；`working/aeonglass053711/bundle/combat-solver/forensics/current/checkpoints/005-export_clicked.json`记录预测20 HP、`combatEndedTurn=null`、敌420；本轮不据此制定整场软目标、不重搜；旧手操后52 HP仅历史证据 | AEONGLASS_BOSS | `CombatSolver-AEONGLASS_BOSS-20260904-053711-747.zip` | `combatsolver-reports-20260905-104549.zip` | 无法参考寄托里的世界线 |
| 9 | 44 HP | 74 → 30 | -1（报告参考） | 否（报告参考） | 软目标达标（20260906）：a5266b4、start完整恢复，High/180秒/DOP4、原Smart，缺项显式补空逐槽指令、两项减战损优先及阈值0；预测/实际31 HP、1药、T6、余49 HP，参考目标52 HP，计划外重算0；搜索缓存false、部署true；证据`.local/checkpoint-batch/worldline-resume/rank9-high-deploy/`，`d8fde8cddb564dbabe712d258eca94d3`；人工30含此前21 HP且无完整人工录制，相对人工仅参考-1 | QUEEN_BOSS | `CombatSolver-QUEEN_BOSS-20260902-113237-991.zip` | `combatsolver-reports-20260905-104714.zip` | 似乎不太会用疯狂之触药水 |
| 10 | 42 HP | 51 → 9 | 未验收 | 未验收 | 按用户新指令暂停，当前先分析包2；本轮`rank10-preflight/`材料有效，High政策已准备，尚未恢复/搜索/部署。旧证据：开战 `VeryHigh / Reported` 请求在 120 秒上限内超时（runId `3040e72109584c14bc8ea5b64df2610e`），未得到战损；人工 9 含此前 T1 已发生的 8 HP，不能直接做整场比较 | KNIGHTS_ELITE | `CombatSolver-KNIGHTS_ELITE-20260904-034250-040.zip` | `combatsolver-reports-20260905-104549.zip` | 更好的世界线 |
| 11 | 40 HP | 62 → 22 | 待验证 | 待验证 | 开战预检通过；原包缺 Boss 政策字段，显式使用 MinimizeHpLoss 后 `VeryHigh` 搜索在 120 秒上限内超时，未得到战损（runId `97fe8ff4092e4e43bb7fe43f015b7b1f`）；不延长等待 | DECIMILLIPEDE_ELITE | `CombatSolver-DECIMILLIPEDE_ELITE-20260902-012309-024.zip` | `combatsolver-reports-20260905-104714.zip` | 更好世界线 |
| 12 | 35 HP | 51 → 16 | 待验证 | 待验证 | 开战预检通过；原包含空药水指令和两类 Boss 政策（均 ProgressionFirst）；`VeryHigh / Reported` 搜索在 120 秒上限内超时，未得到战损（runId `68c7faaf4a0d4c39b3542983a6bcc816`）；人工记录含手动推进三回合后的后续求解，暂不作整场比较 | KAISER_CRAB_BOSS | `CombatSolver-KAISER_CRAB_BOSS-20260904-231535-471.zip` | `combatsolver-reports-20260905-104412.zip` | 提示打不过boss，手动打了三个回合后计算就能打过了 |
| 13 | 33 HP | 51 → 18 | 待验证 | 待验证 | 开战预检通过；原包含空药水指令但缺两类 Boss 政策；显式 MinimizeHpLoss 的 `VeryHigh` 开战搜索完整胜利但预测战损 88 HP（118.03 秒，runId `8c1218795af349d48685e523fcc30cb8`），确认是策略缺口而非还原失败；人工 18 HP 的后续局仍不能直接作为整场对照 | AEONGLASS_BOSS | `CombatSolver-AEONGLASS_BOSS-20260902-131206-788.zip` | `combatsolver-reports-20260905-104714.zip` | 上来给我算死了，操作一下中间重算好几次，然后只用掉18 |
