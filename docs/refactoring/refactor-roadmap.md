...
本页早期路线已完成并合并到 `main`；下列追加批次以各自证据的状态为准。

2026-09-20：[搜索内存生命周期重构](search-memory-lifecycle-20260920.md) 已完成所有权迁移与失败完成链修复、两端124项CLR检查和36份Windows配对验收；全部正负差异及限制见报告，不提升版本、不发包。
