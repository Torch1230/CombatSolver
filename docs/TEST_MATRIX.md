...
| 场景 | 结果 | 验证内容 | 日期 |
| --- | --- | --- | --- |
| `UI-MEMORY-GC-WALL-IDLE` | 待玩家实测（此前结构验证通过） | 内存框取消固定宽度并填满底栏剩余空间，使用 `8 px` 进度条；搜索中显示“负荷/即将整理/正在整理”，待机显示预算占用，常规 GC 显示“自动管理”，后台 Gen2 显示“后台清理中”。Smart 药水梯度切换时仅保留轻量路线并从新检查点计量。此前结构 runId `9cc48cec6a4648beb701711e59f09b8c`；本轮按要求不运行 UI 测试。 | 2026-09-02 |
