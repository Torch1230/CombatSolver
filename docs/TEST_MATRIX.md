...
## 可恢复选牌研究（2026-09-13，非生产实现）

- 理论收益分析只运行确定性Python模型和输入/输出检查，复用既有CPU、ABBA及普查证据；无新游戏运行、无生产变更。公式、跨基线假设与GC/并行限制见[收益报告](performance/resumable-choice-benefit-20260913.md)。

- 诊断Release构建通过，最终增加类型/联合阻挡计数的构建零警告/错误；临时注入源码构建后恢复。两轮 `73d855dc513f4d86bcc32ae7bc672561` / `ebf379e1a6494fe893495be05cfaddf6` Passed，83个非运行时/非调度字段、40完整动作、路线文本与原B1一致，规范化Fork349198。使用3b91d22、20000节点、两药水槽禁用，仅作形状/成本普查。
- 0.38来源1ebd323的 `fee6d5b066df46a584825e05fea76cf8` 在搜索前因旧卡牌ContinuationStamp缺少keywords字段失败；未旁路对账。
- [调用链/资格/计数](performance/resumable-choice-research-20260913.md)、[结构化证据](performance/resumable-choice-research-20260913.json)、[重跑方法](../tools/ChoiceContinuationProbe/README.md)。尚未实现续接，未运行分支隔离、DOP续接合同及无插桩性能A/B，不是完整原包验收。
