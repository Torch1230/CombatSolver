...
## 下一补丁：录像对局导入（开发中，2026-09-14）

- 使用本次实际下载的 `ShowcaseBundleV1` 在 Windows 上验证五个协议文件全部解压，其中四个负载文件均在关闭写句柄后通过大小与 SHA-256 校验，未再出现共享冲突。Release 构建通过，0 警告、0 错误，并更新本地游戏目录。游戏内进入临时对局由玩家重启加载新 DLL 后验证；本轮不使用 Computer Use、不启动可见 Steam。
