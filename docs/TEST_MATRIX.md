...
## 2026-09-21：撤回4GB上限后的Windows验证

当前#116 `9645eba8` / #120 `7479db3a` 与原验收基线 `3e8fb5ce` 六场三组交错，共36份有效搜索，均到达固定节点边界，24对完整已记录路线、非时序字段及开局/执行政策一致。Windows CLR分别52/61项通过。另有一次基线资源预加载访问冲突，保留失败后同输入重试通过。两候选仍有约2秒最大GC暂停；#116弃牌均值+8.45%，#120相对#116首领耗时+10.57%、亡灵工作集峰值均值+13.32%。性能目标未达成，不建议直接合并。全部样本、限制和清理见[本轮报告](performance/no-gc-budget-restored-20260920.md)。本批Windows根已删除、所属进程0，未修改生产代码/正式安装、未发包。


## 2026-09-20：撤回 Windows NoGC 4GB 硬上限

按用户要求，证据不足的区域大小政策恢复上游行为：删除 Windows 专用截断及其专属断言，直接使用用户配置进入原有准入流程；保留系统余量、CLR限制、回退与恢复约束。核对上游 `3f4002bd` 在入口直接使用配置预算，没有该截断。中英文设置说明、架构与性能维护规则同步修正。旧 4GB 样本保留为历史数据，不作为此次预算恢复后的性能结论。当前 Windows 搜索耗时、内存与暂停表现尚未重新验证。 本次 Linux Release 构建0警告0错误；基础26项（含准入6项）、真实CLR检查点1项、恢复生命周期2项通过，中英文修改项映射检查通过。两条PR分支分别执行；未运行Windows或游戏性能对照。


## 下一版本（开发中）：搜索内存生命周期重构（2026-09-20）

- 单次scope拥有恢复退避和预算，值类型回收状态拥有phase/完成源/收集元数据；提交准入和GC模式决定收口到Runtime，删除重复状态与Search直接恢复控制。保留现有搜索与NoGC数值政策。
- 修复诊断异常卡住回收完成链：相同首项夹具在冻结基线超时，候选五个故障边界传播异常并完成清理。最终Release构建0警告0错误、两端结构门禁、Linux/Windows各62项CLR检查及Linux代表搜索通过。
- Windows六场ABBA加三场定向BAAB，36份Passed，18对非时序指标/记录路线/开局与执行政策一致。首批耗时−1.20%～+1.95%；三场复核−0.61%～−0.03%，不宣称提速。重场景初批私有峰值+7.80%/+3.34%，复核+0.05%/−3.58%；全部样本与余量回退差异保留，不把波动当作零退化保证。见[实现与验收](refactoring/search-memory-lifecycle-20260920.md)。
- 两端无人启动器在隔离profile关闭四路音量并使用Dummy音频驱动。Windows本批测试根、实例、工具与临时产物已清理，相关进程为0；Linux代表实例已删除。原PR #116及正式安装保持，不提升版本、不发包；未做可见帧时间或整场原生部署验收。
...
- 原包固定 8,000 节点/成员、DOP1、60 秒预算，独立进程 A-B-B-A 四次均 Passed/NodeLimit；总展开 17,600、转移 217,538、预计战损 1，两对各 60 项非时序指标一致。请求搜索分配均值 −3.25%，累计搜索耗时均值 −0.97%；两侧测试默认 No-GC 16 GB，不是原机 12 GB 压力验证。
- `GARDEN-ALLOCATION-NATIVE`：`coverage/unattended/garden-allocation-native.json` 的 7 项原生差分通过（24.46 秒），包含花园幽灵鳗四种行动、多段攻击首次匹配与格挡、实验体动态次数。怪物差分入口新增全部捕获行动、父/兄弟隔离、已返回列表稳定和根外眩晕回退合同。
- 离线哨兵 `sel-defect-elite-02`：86 项工具比较通过，并单独核对完整动作/选牌、全部续用文本、根戳、剪枝计数和政策一致。分配略降，单对墙钟约 +0.8%，不宣称速度改善。
- 前后目标类型/调用栈采样均严格解析，缺栈/未解析栈/丢事件均为 0；目标静态意图和受攻击工厂闭包分配消失。Release 构建与结构门禁通过，所有无头实例由启动器清理。
- 未做：原包完整 300 秒、DOP16、Windows、可见 Steam、完整自动部署。完整口径、命令与原始产物位置见[报告](performance/garden-allocation-20260919.md)及[结构化证据](performance/garden-allocation-20260919.json)。
