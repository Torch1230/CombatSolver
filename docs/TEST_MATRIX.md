...
- 实体、trace 与模拟器外壳是测试替身；真实战斗、完整 simulator Fork、控制器采用／缓存／部署以及 T1→T2 差分未执行。配置变化证明标记失配，Runtime 的接线通过结构门禁约束，不冒充实机部署证明。
- Release 编译使用游戏 0.111.0 与对应 RitsuLib，不部署到游戏目录，不运行性能 A/B。
