...
- 部署高亮过渡：`UI-LOCALIZATION` 的循环高亮合同在比较颜色前先收敛过渡，并新增节奏合同（未知节奏与 `1.5 秒` 间隔取 `0.08 秒`、`0.2 秒` 间隔取 `0.04 秒`、`0.02 秒` 间隔直接切换、呼吸延迟内为 0、满幅峰值 >0.99）；行复用合同仍要求复用后立即全白。两项合同未在无头实例重跑（本机无 Linux 无头入口），构建与结构门禁通过。
