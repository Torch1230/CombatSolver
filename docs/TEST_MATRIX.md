...
- `UI-COMPACT-QOL` Passed（runId `7412946df0ca4088aa53cf5bfc4bf06b`）：全自动开而自动计算关、药水/成长/遗物各自关闭、累计受伤/已回血或生命提升/后续预计回血/净变化，以及显示用恢复量公式。模拟不支持 NoGC 的入口核对未调用区域启动且未取得/恢复延迟模式所有权；该模拟不等于 Android/iOS 真机验证。随后净变化改为单独按正负着色的标签，最终版复跑结果在本节续记。`UI-PRIORITY-FEEDBACK` 最终复跑 Passed（runId `508238b1cbd74c21bbb41f7459118d52`）：展开和小窗“采用／执行／冻结”各自的可见与禁用状态合同通过。
- 最终净生命标签 `UI-COMPACT-QOL` 复跑 Passed（runId `44dd21643d604c6a95c1a0527aa816fe`）：0、负、正三种净变化独立显示，负值用受伤色、正值用回血色；子面板关闭和模拟手机 GC 合同一并复跑。Release 完整构建 0 警告/错误；Windows 结构门禁 `REFACTOR_BOUNDARIES_OK search_files=204`。
- `UI-LOCALIZATION` Passed（runId `cfef05776e8a4800847a941958484e8f`）：435 项中英目录（eng/zhs/zht）占位符一致、带符号净生命变化格式及既有卡名/注解切换合同通过。以上均在隔离无头环境；可见界面的遮挡和点击体验未验收。
