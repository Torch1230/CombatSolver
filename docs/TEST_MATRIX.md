...
## 当前交付验收摘要（2026-09-06，待同步主线后验证）

用户本次明确允许少量战损或回合数回退，停止为恢复全部旧回合目标继续试验；验收重点是通用正确性、避免明显战损及严重性能退化。外骨骼虫零损 T10 按此口径接受，旧零损 T5 仍是已知质量差距，不再作为本次必达目标。下方历史 FAIL、实验撤回和暂停结论仍按当时标准保留；协议 Passed、模拟回放通过、首结果质量、实际部署与性能证据互不替代。

v77 结果只属于合并前源码；最新主线撤回旧 worldline 估值/保路，因此正常合并后的 v78 才进入最终验证。以下结果不能自动转记为合并后通过，最终定向测试需重跑；不为此次同步引入新的策略实验。
...
| 合并后最终定向质量与性能抽查 | 待主线同步后的 v78 候选执行；最终提交与 runId 待补充 | 暂不声称最终候选已通过；v77 结果、既有 headless 时间和 v52 GC A/B 不代替合并后或可见 Steam 验证 |

此前已执行的最小事务/Fork/终局差分、v52 后台 GC 八组合同与固定工作量 A/B、v67 同回合落选恢复合同、v66 自定义战斗正常自动部署仍可按各自适用边界引用。未因纯文档整理重复执行；完整发布门禁、全量 CoverageCatalog、可见 Steam 卡顿和 Windows 原生游戏验收未在本轮完成。PR 交付说明见 [通用搜索与正确性后续 PR](pr/generic-search-correctness-followup.md)。
