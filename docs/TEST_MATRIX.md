...
- 连续威胁排序：全部中途节点版本20根4好/1差，拒绝；仅新回合起点版本20根3好/0差/17同，验证集前25根2好/1差/22同（抽牌高压4→5 HP/T8→T9），仍在完成其余根，不声称训练外零退化。两版源码均编译0/0，终局/转置政策保持，默认关闭。
- 当前证据是离线测量与纯值合同，没有可见 Steam 或原生新行为验收；复现命令和后续结果见[实验记录](strategy/contextual-ordering-20260922.md)。
