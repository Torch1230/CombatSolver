...
> 基线：CombatSolver `0.25.1`（当前创意工坊稳定版；下一版本待定）、塔 2 `0.111.0`、RitsuLib 实测 `0.5.18`（清单最低 `0.5.13`）、CombatSolver 内置战斗模拟引擎。无人测试运行隔离的原版 `--headless` 游戏进程，不使用自建 STS CLI；性能最终门槛另由 Steam 可见会话验证。完整战斗基准使用 `Instant / 0 秒` 部署。
...
| 场景 | 结果 | 验证内容 | 日期 |
| --- | --- | --- | --- |
| `CUSTOM-LOOP-BLOCK-PAYOFF-DEPLOY-V1` | 通过（同根 headless 完整自动战斗） | 只以可由全身撞击兑现的当前格挡打破 Beam 完全同分，不修改普通候选的原排序；跨 `7` 次洗牌，以 `19` 动作在第 `1` 回合击杀，最终 `1 HP`、战损 `3`，实际使用火焰药水。runId `2a543eced76a48a0850086419b588964`。 | 2026-09-02 |
| `EXOSKELETONS-BLOCK-PAYOFF-SENTINEL` | 通过（性能集成分支 headless 搜索） | 无全身撞击牌组保持 `5,686` 展开、`199,522` 转移、`175,150` 选择分支及原动作路线；最终 `96 HP`、战损 `1`、第 `4` 回合结束。runId `1ace1bd7219d48e1ab367ae7e7f75957`。 | 2026-09-02 |
