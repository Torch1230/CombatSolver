...
> 基线：CombatSolver `0.27.0`（当前创意工坊稳定版）、塔 2 `0.111.0`、RitsuLib 实测 `0.5.18`（清单最低 `0.5.13`）、CombatSolver 内置战斗模拟引擎。无人测试运行隔离的原版 `--headless` 游戏进程，不使用自建 STS CLI；性能最终门槛另由 Steam 可见会话验证。完整战斗基准使用 `Instant / 0 秒` 部署。
...
| 场景 | 结果 | 验证内容 | 日期 |
| --- | --- | --- | --- |
| `UI-MEMORY-GC-WALL-IDLE` | 通过（headless UI、控制器生命周期、DOP4） | `220 px` 紧凑内存框使用 `8 px` 进度条；活动 NoGC 搜索按实际回收检查点变色并显示“内存回收中”，待机和常规 GC 按工作集占配置预算的比例持续填充，后台 Gen2 显示“后台回收中”。Smart 药水梯度切换时仅保留轻量路线并从新检查点计量。新默认尺寸 `1200 × 700`、透明度 `65%`。runId `9cc48cec6a4648beb701711e59f09b8c`。 | 2026-09-02 |
| `UI-SEARCH-LIMIT-WARNING` | 通过（headless UI、控制器生命周期、DOP4） | `TimeLimit` 与 `NodeLimit` 结果始终显示不可关闭的顶部警告，分别指出时间/节点上限与性能设置；正常结束不显示。runId `0482e473ee9b4271ba314c25fa9285a7`。 | 2026-09-02 |
