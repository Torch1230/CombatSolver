...
- 离线宿主 `OFFLINE_HARNESS_CHOICE_TOPK_VERIFY=1`（High 90/50000、Coordinator、Smart、DOP 1）：搜索期间每次多选生成后独立完整枚举并按现评分排序，核对保留集恰好是前 K。2 个 EQ 根加 4 个多选根（储君 `GUARDS`/`CHARGE`/`GLIMMER`/`PHOTON_CUT`，亡灵契约师 `DREDGE`/`SEANCE`/`CLEANSE`，静默猎手 `PREPARED`/`HIDDEN_DAGGERS`，外加无色 `PURITY`）全部跑到 M2，无断言失败。
- 对照 0.43.2（`3f4002b`）：EQ 10 根加 12 个多选根，`compare_results.py` 2,054 字段。12 根逐字段一致，10 根不同；**不同的 10 根都发生过截断，没有发生截断的根全部一致**（截断次数取自此前对同一批根的普查）。10 根里胜负无变化；最终路线的战损或用药变化 2 根：`CHOICE-NECROBINDER-BOSS-00` 未胜路线战损 52→47，`CHOICE-REGENT-ELITE-00` 由 22 战损用 1 瓶药变为 32 战损不用药。22 根合计总展开 +0.49%、总转移 +1.05%。
- 改动前的普查（0.43.1，72 根）：165,436 次多选生成里 12.0% 触发截断，被截掉的组合占全部语义组合的 18.9%，集中在从牌堆取牌和变形，弃牌路径 0 次；单次语义组合数最大 512，完整枚举加评分占求解墙钟 0.05%。
- 质量对照（0.43.1 上的实验实现，20 个受影响的根，生产最终路线比较器全部排序键）：5 根更好、13 根持平、2 根更差，无胜负翻转；符号检验单侧 p≈0.23、双侧 p≈0.45，不足以确认战斗质量提升。同一搜索对「完全同分候选次序反转」这种中性扰动也会改变约一半根的最终路线，所以这组数字只能说明没有观察到退化。
- 只在离线宿主验证，未在游戏内验证。Windows 未验证。
