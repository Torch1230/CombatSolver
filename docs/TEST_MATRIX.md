...
## 未发布：多策略路线搜索默认开启（2026-09-17）

- 设置与 UI 合同已更新：新安装默认开启多策略路线搜索；244→245 迁移会开启功能并显示一次功能横幅；245 之后玩家关闭功能或横幅的选择保持不变。横幅仅在功能开启时显示，说明可从「设置 > 性能」关闭，并维护等价英文文案。
- `NOVELTY-PORTFOLIO-SETTINGS` 首次运行到设置断言后失败：旧合同仍要求 244→245 再次强制开启多宽度路线精炼。正确边界是 244 已完成该迁移，245 应保留玩家之后对精炼的选择，只统一开启多策略搜索；已修正合同，不归为生产迁移缺陷。
- `NOVELTY-PORTFOLIO-SETTINGS` / `f6cc044d5e66472aba1494cc37f2ae36` Passed，22.67 秒：验证多宽度精炼与多策略搜索均为新安装默认开启；244→245 只强制开启多策略并显示横幅；当前版本关闭精炼、多策略或横幅后均保持玩家选择；性能页控件、设置往返和请求冻结一致。
- `UI-LOCALIZATION` / `da26aa3d36844646b4ffb5e787f9de76` Passed，25.81 秒：eng/zhs/zht 共 426 项目录，功能横幅在多策略开启时显示，文案明确默认开启与关闭位置，点击后永久隐藏且不关闭功能；关闭功能时横幅不显示。Release 编译 0 警告、0 错误，PowerShell 结构门禁 `search_files=114` 通过；未启动可见 Steam。

## 未发布：请求级搜索进度（2026-09-17）
