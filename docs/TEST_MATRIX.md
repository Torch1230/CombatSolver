...
- 两个源码原型撤回，现成实验保持关闭；恢复后Release零警告/错误。不为否决原型追加原生验收；一次预算回收测量与短算术检查重叠，排除其性能结论。详情见[报告与结构化证据](performance/bounded-quality-screen-20260922.md)。
...
- 新增一次性义务合同直接调用生产helper，覆盖节点/父节点变化、同键别名，以及不同子状态/父状态/来源。原生请求先通过SearchPolicySnapshot（含DOP1/DOP2、取消与错误排空），随后Fork历史合同失败；同请求未修改基线复现。Regent严格回放的FURNACE前缀差分也在两边复现。两项均保留为Failed，未解决；候选失败进程另有退出SIGSEGV日志，不隐藏。
- 原生极高DOP16 `37c9bdf2a8c7455ebf09ccd8402f998d` Passed，总60,000展开/956,156转移、实际并发16；Defect严格回放 `a2d7bcde3a7745d1b7bf08d43f1e9b5f` Passed，配置300节点、总545展开/2198转移。六次原生实例均确认删除。Release零警告/错误，Bash结构门禁208；未验证Windows、可见帧时间和整场部署。

## 完整回合层之间回收（2026-09-22）

- 最终二进制24根48次主A/B，覆盖五角色/精英/首领/普通遭遇、弃牌/药水及5个新Act2种子；另有DOP4、1 GB、普通GC、8秒软预算各2对，以及dev05修复重测与普通GC反向顺序复测各1对，共34对68次。完整路线/质量/根/政策及去除计数后的搜索结果相等。主语料23对全部搜索/剪枝字段相等，新增Ironclad一根触及60秒预算，补充药水搜索展开114754→114000但路线相同；小区域缓存、并行捕获与短预算差异单列。
- 生产GC合同64项通过（基础/准入26、scope8、checkpoint1、between-searches10、恢复9、恢复生命周期2、诊断失败8），覆盖独立退避、每solver重试、mandatory额度隔离和真实排队期间回收/重开；Release主项目与GC工具零警告/错误，Bash结构门禁208。初版共享退避造成dev05 RSS退化19.89%，已修正并重新跑全部主语料，旧原型不混入最终结果。普通GC反向样本未隐藏。详见[报告](performance/turn-layer-reclaim-20260922.md)和[结构化证据](performance/turn-layer-reclaim-20260922.json)。
- 原生NoGC回收 `4d3485abf39149afa89201be71d97b94`（4000总展开、回收/重开各3、区域丢失0）与Defect严格增量 `f9b0f16912fc489280741f0868337d86`（配置每solver300节点、总展开544）均Passed且实例删除。前者配置DOP2但实际并发0；并行证据来自两根离线DOP4的实测并发4。未测Windows、可见Steam或完整战斗部署。

## 计算过程中的搜索间回收（2026-09-22）

- 17根34次主A/B覆盖五角色/精英/首领/留出及弃牌/药水；最终版本另有DOP4、1 GB NoGC、普通GC、8秒软预算各2对，主配置重复3对，共28对56次。完整路线/质量/根/政策及去除计数后的搜索结果全部相等；主语料一根仅缓存数量不同，截止时间和DOP计数差异单列。
- 生产GC源码合同61项通过（含新增搜索间检查点7项），Release主项目/工具零警告错误，Bash结构门禁208。初版并行NoGC准入夹具超时后按既有串行契约修正，原失败记录保留。
- 原生首次跳过 `61f2ce1a8f4a43dc98ec857871b3c553`、Defect严格增量300节点 `58fad8e9b366401ca20dccabe5bc728d`、Regent DOP2 4000节点/真实回收重开 `eda4cb6d8d9f4e6bb194197ef975d045` 全部Passed且实例删除；最后一项区域丢失0。峰值/时间/暂停的取舍、输入覆盖修正与未测范围见[报告](performance/active-search-gc-20260922.md)及[结构化证据](performance/active-search-gc-20260922.json)。未执行可见Steam、Windows或完整战斗部署。

## 搜索进度引用释放（2026-09-22）

- 40项生产类型生命周期合同通过，旧源码91e12e88精确负对照失败。17根34份最终A/B覆盖五角色/三类遭遇及弃牌/药水；12根停止、5根正常完成，全部新版旧seed可回收、停止结果原样恢复。额外DOP4两根4次通过，5个正常完成根共10对完整路线/质量/根/政策/剪枝全等。
- 原生停止后采用、停止/重算/部分手选恢复、重算接管3项通过；通用控制器fixture在候选/基线均因相同UI尺寸持久化断言失败，未验证其后续停止链。5次原生启动全部确认清理实例。Release主项目/宿主零警告错误，Bash结构门禁208。完整数据、runId、命令与探针修正记录见[报告](performance/progress-retirement-20260922.md)及[证据](performance/progress-retirement-20260922.json)。未执行可见Steam、Windows或完整自动战斗。
