...
- 永久成长默认 3 点、净资源默认 25 分，并分别提供 UI 输入。旧配置或显式输入 0 采用有限默认，不再表示无限收益；已设置的正目标值保留。
- 两种目标都封顶目标排序值。只有完整胜利、实际收益达标且 HP 限制满足时触发结束；Beam 在现有层边界收尾并保留当前已找到路线，不中断正在进行的模拟动作。协调器的满足条件判断同步支持目标达标，药水梯度在启动下一层前和取得新结果后检查。
- 平衡／保命规则不变；不完整路线、无收益或收益越界不触发目标提前停止。高于目标的实际收益仍用于明细显示。每次重新搜索重新计量，尚未增加跨回合累计培养配额。
- `tools/SearchObjectiveChecks` 227 项纯目标／排序断言通过，新增旧值 0 默认、两种目标封顶、完整胜利与 HP 条件、平衡不受影响。未执行原生搜索／无限循环问题包／部署验证，不能宣称已复现并解决用户当前卡住的现场；游戏内停止时机和布局待验证。
