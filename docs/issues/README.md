# 问题批次与分诊

- [回合末 COW 卡牌接收者失效](regret-bound-hook-receiver-20260910.md)：146 份计算失败分诊与Regret/束缚接收者修复。

[返回文档导航](../README.md)

新问题包的导入与验证方法见 [检查点回放](../CHECKPOINT_REPLAY.md)。下列批次按其记录日期和版本保留，计划项不等于已完成修复。

- [2026-09-09 22:56 实验体批次：34 份报告与原始击杀／复活错误修复证据](test-subject-reports-20260909.md)。

- [2026-09-09 计划外重算：0.34.4 未修复报告分诊](unexpected-replans-20260909.md)。
- [2026-09-08 计划外重算：188 份报告分诊与高频共性修复](report-replans-20260908.md)。

- [2026-09-07 修复暂停交接：剩余问题与继续入口](report-logic-bugs-20260907-handoff.md)。
- [2026-09-07 新召唤自爆敌人导致搜索失败](living-fog-summon-20260907.md)。
- [2026-09-07 三个汇总日志包：逻辑问题分诊与修复](report-logic-bugs-20260907.md)。
- [2026-09-04 19:00 后问题包分诊](ISSUE_TRIAGE_20260904_AFTER_1900.md)。
- [2026-09-03 硬逻辑问题分诊](20260903-hard-logic-triage.md)。
- [0.21.0 修复批次](post-0.19.0-pending-bugs.md)。
- [0.18.0 发布后问题批次](post-0.18.0-bug-batch.md)及[修复计划](post-0.18.0-fix-plan.md)。
- [0.18.0 原始问题](0.18.0-raw-bug-batch.md)及[集中修复计划](0.18.0-bugfix-plan.md)。

- [玩家死亡能力清理遗漏（2026-09-10）](player-death-power-cleanup-20260910.md)

- [伤害来源死亡状态读取实机（2026-09-10）](shadow-damage-dealer-liveness-20260910.md)

- [动态目标类型读取实机状态（2026-09-10）](branch-local-card-targeting-20260910.md)

- [结束回合循环出口准入遗漏（2026-09-10）](end-turn-cycle-admission-20260910.md)
