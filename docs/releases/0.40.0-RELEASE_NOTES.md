...
- 新增默认关闭的“多策略路线搜索（实验）”。启用后，求解器会先用一部分现有预算探索结构不同的打法，再用剩余预算进行常规搜索，并继续按当前的战损、成长、药水和复活规则选择最终路线。该功能不会突破设置的时间和节点上限，但可能改变搜索耗时和最终结果。
- 修复战斗录像导入时临时能量费用、临时星能费用和故障机器人充能球状态恢复不完整的问题。录像中的卡牌费用与充能球现在会按记录状态准确恢复，旧球位和球体会在重建前完整清理，不再残留重复或悬空的球体画面；导入失败时也会保留最初的错误原因。
