...
> **分支状态（2026-09-12）：暂时不开发。** 用户已暂停 `perf/simulation-profile-20260910` 的整套 Compact／战斗模拟移植，原因是工作量与收益不成正比。保留历史代码与证据，不继续补齐后端、状态族或卡牌。新的工作重点为从最新上游建立的 `perf/surgical-fixes-20260912`，只接受可独立验证的小修复；恢复本分支须有用户新指令。
