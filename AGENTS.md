...
> **当前工作重点（2026-09-14）：** `perf/general-allocation-20260914` 已快进整合上游 `b1674f8`（0.38.2），完成本批五项通用分配/重复工作优化；完整极高对照、峰值反例、逐场复核及高成本待审项见 `docs/performance/general-allocation-20260914.md`。后续仍保持搜索预算、决策质量及完整动作，逐场检查耗时/峰值而非只看累计分配。历史PR #90已合入；可恢复选牌执行器仍属独立高成本草稿，不混入本批。不重写模拟器、不大幅重构、不提升版本、不发包或上传。
