...
> **当前工作重点（2026-09-14）：** 将已整合 PR #85–#88 与 PR #92 的 `main` 定版为 0.38.2。玩家版本仅说明第三方适配接口；通用战斗生成器属于测试工具。正式发布使用统一脚本同步创意工坊、GitHub Release 与夸克网盘，监控后台最新版提示由用户维护。
...
- GitHub Release 的最小 ZIP 统一写入仓库根目录的 `releases/CombatSolver-<版本号>.zip`。夸克网盘由统一脚本另建 `releases/CombatSolver-<版本号>-Quark.zip`，在最小包基础上原样加入 `releases/STS2 RitsuLib 0.5.20.zip`，且总大小必须严格超过 10 MiB；前置 ZIP 不解压。当前工作区发布目录为 `D:\Desktop\sts2mod\CombatSolver\releases`，所有发布产物均由 Git 忽略。
