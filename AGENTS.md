...
> **当前工作重点（2026-09-13）：** 用户要求已完成性能优化由PR #90正式评审，可恢复选牌执行器在`perf/resumable-choice-executor-20260913`独立草稿推进。当前是设计及单一抽弃牌路径原型范围；先吸收#90最终合入实现，按`docs/performance/resumable-choice-executor-design.md`分阶段验证。不得直接推广通用执行器或移植Compact，不改变搜索预算、候选顺序或决策质量；本批不发版、不启动可见Steam。
