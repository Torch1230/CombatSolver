...
> **当前批次（2026-09-20）：** 用户授权在 `refactor/search-memory-lifecycle-20260920` 持续完成搜索内存/GC生命周期重构，基线 `b562f249`，保留PR #116。整体交付不得变慢、增加内存或降低决策质量；只有量化巨大性能收益与极小质量代价时才将非等价方案单列供用户判断，所有退化必须披露。目标是减少实际GC协调机制，不以拆文件代替。见 [重构计划与验收](docs/refactoring/search-memory-lifecycle-20260920.md)。本批不提升版本、不发包、不启动可见Steam；Windows本批测试资源最终全部清理。
