...
- GitHub Release 的最小 ZIP 统一写入仓库根目录的 `releases/CombatSolver-<版本号>.zip`。夸克网盘由统一脚本另建 `releases/CombatSolver-<版本号>-Quark.zip`：CombatSolver 最小包内容保留在根目录，`releases/STS2 RitsuLib 0.6.0.zip` 的内容解压写入根目录下的 `RitsuLib/` 文件夹，不把整份前置 ZIP 嵌套进最终包；总大小必须严格超过 10 MiB。当前工作区发布目录为 `D:\Desktop\sts2mod\CombatSolver\releases`，所有发布产物均由 Git 忽略。
